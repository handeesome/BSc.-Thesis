...
def produce_centralities(net, layer, threshold):
    ...
    degree_centrality_df = pd.read_csv('biogrid_degree_centrality_7.604545773719985.csv')

    top_nodes = degree_centrality_df.nlargest(100, 'degree_centrality')['node'].tolist()

    
    clsns_centrality = cc.closeness_centrality(nx_net.subgraph(top_nodes))
