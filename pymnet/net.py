...
class MultilayerNetwork(object):
    ...
    def _write_flattened(self, output):
        nodes = map(lambda x: tuple(reversed(x)), sorted(itertools.product(
            *map(lambda i: sorted(self.slices[i]), reversed(range(len(self.slices)))))))
        for i in nodes:
    ...
    def get_supra_adjacency_matrix(self, includeCouplings=True):
        ...
        import numpy
        if self.aspects > 0:
            nodes = list(map(lambda x: tuple(reversed(x)), sorted(itertools.product(
                *map(lambda i: sorted(self.slices[i]), reversed(range(len(self.slices))))))))
...
class MultiplexNetwork(MultilayerNetwork):
    ...
    def __init__(self, couplings=None, directed=False, noEdge=0, fullyInterconnected=True):
        ...
        if isinstance(
                couplings, tuple) or isinstance(
                couplings, "".__class__) or isinstance(
                couplings, u"".__class__) or isinstance(
        ...
        if len(d) == 1:  # not a self-link, or link with multiple different cross-aspects
            if d[0] > 0:
                ...
                if not self.fullyInterconnected:
                    ...
                    if not (link[0] in self._get_A_with_tuple(supernode1[1:]).
                            slices[0] and link[0] in self._get_A_with_tuple(supernode2[1:]).slices[0]):
                        return self.noEdge
    ...
    def _get_dim_degree(self, supernode, aspect, direction="tot"):
        ...
        if coupling_type == "categorical":
            if self.fullyInterconnected:
                ...
                if layer in self._nodeToLayers[supernode[0]]:
                    if self.aspects == 1:
                        ...
                        return len(filter(lambda x: x[aspect] == supernode[aspect],
                                          self._nodeToLayers[supernode[0]])) - 1
                else:
                    return 0
            ...
            if self.fullyInterconnected:
                ...
                return int(supernode[: aspect] + (up,) + supernode[aspect + 1:] in self._nodeToLayers[supernode[0]]) + int(
